...
# =============================================================================
# Parser para formato DF (IDFP) - Data Flow
# Formato do out.txt:
#   List(Node(<ClassName: RetType method(params)>,stmt,line,NodeType, path: (...))
...
def parse_and_match_stream(txt_path, csv_index, output_csv_path):
    """
    Processa o out.txt no formato DF, extrai Nodes e busca matches no índice CSV.
    """
    print(f"Processando TXT (formato DF): {txt_path}")
    ...
    with open(output_csv_path, mode='w', newline='', encoding='utf-8') as f_out:
        ...
        with open(txt_path, encoding='utf-8') as f_in:
            for line_idx, line in enumerate(f_in):
                ...
                # Encontrar o SourceNode para construir o entry_dict
                source_node = None
                ...
                entry_dict = {
                    "Kind": source_node[3],
                    "ClassName": source_node[0],
                    "MethodName": source_node[1],
                    "SourceCodeLineNumber": int(source_node[2]),
                    "UnitStatement": source_node[4],
