...
def extract_method_name(method_full):
    match = re.search(r'\b(\w+)\s*\(', method_full)
    if match:
        return match.group(1)
    else:
        # Se não encontrar parênteses, tenta pegar a última parte depois do ponto
        return method_full.split('.')[-1].strip()
